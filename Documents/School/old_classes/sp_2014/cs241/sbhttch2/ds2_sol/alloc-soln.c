...
void *malloc(size_t size) {
    ...
    chosen = sbrk(0);
    sbrk(sizeof(dictionary_entry_t));
    chosen->ptr = sbrk(0);
    if (sbrk(size) == (void*)-1) {
        return NULL;
    }
...
void *realloc(void *ptr, size_t size) {
    ...
    dictionary_entry_t *old_p = dictionary;
    while (old_p) {
        if (old_p->ptr == ptr) break;
        old_p = old_p->next;
    }
    if (old_p && old_p->ptr == ptr) {
        if (old_p->size >= size) {
            old_p->free = 0;
            return old_p->ptr;
        }
