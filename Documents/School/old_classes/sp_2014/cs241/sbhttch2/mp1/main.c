...
int main()
{
	...
	else { printf("_add()1: OKAY!\n"); }
	
	result = dictionary_add(&dictionary, "key", "value");
	if (result != 0) { printf("add should have failed!.\n"); }
	...
	dictionary_remove(&dictionary, "key3");
	s = dictionary_get(&dictionary, "key3");
	if (s == NULL) { printf("trying to remove already removed key.\n"); }
	else { printf("_get(): OKAY!\n"); }
	...
	char *s3 = malloc(1000);
	strcpy(s3, "http: //www.w3.org/Protocols/HTTP/1.0/draft-ietf-http-spec.html#Message-Headers ");
	result = dictionary_parse(&dictionary, s3);
	if (result == 0) { printf("%s\n",dictionary_get(&dictionary,"http")); }
	else { printf("_parse(): FAILED!\n"); }
