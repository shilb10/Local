...
int dictionary_add(dictionary_t *d, const char *key, const char *value)
{
	...
	{
		for(dictionary_entry_t* it = d->entry; it != NULL; it = it->next)
		{
		...
		dictionary_entry_t* finalEntry = d->entry;
		while(finalEntry->next != NULL)
		{
			finalEntry = finalEntry->next;
		}
		
		finalEntry->next = (dictionary_entry_t*)malloc(sizeof(dictionary_entry_t));
		finalEntry->next->key = key;
		finalEntry->next->value = value;
		finalEntry->next->next = NULL;
	}	
	return 0;
}
...
{
	int colon = 0;
	while(colon < strlen(key_value) && key_value[colon] != ':')
	{
		colon++;
	}
	if(colon >= strlen(key_value) || colon == 0)
	{
		return ILLEGAL_FORMAT;
	}
	if(key_value[colon + 1] != ' ')
	{
		return ILLEGAL_FORMAT;
	}

	key_value[colon] = '\0';
	if(colon == strlen(key_value)-2)
	{
		/*change added. handles empty values*/
		dictionary_add(d,key_value,'\0');
	}
	{
		dictionary_add(d, key_value, key_value+colon+2);
	}
...
void dictionary_destroy(dictionary_t *d)
{
	...
	dictionary_entry_t* curr = d->entry;
	dictionary_entry_t* next = curr->next;
	while(next != NULL)
	{
		free(curr);
		curr = next;
